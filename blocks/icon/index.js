// Wait for wp to be available (for WordPress Playground compatibility)
(function () {
  function registerBlock() {
    if (typeof window === 'undefined' || typeof window.wp === 'undefined' || !window.wp.element || !window.wp.blocks || !window.wp.blockEditor || !window.wp.components || !window.wp.i18n) {
      // Retry after a short delay if wp is not yet available
      if (typeof window !== 'undefined') {
        setTimeout(registerBlock, 100);
      }
      return;
    }

    // wp is available, register the block
    const {
      createElement,
      Fragment,
      useState
    ...
    registerBlockType('webentwicklerin/icon', {
      edit: ({
        ...
        return createElement(Fragment, null, createElement(InspectorControls, null, createElement(PanelBody, {
          ...
          options: [{
            label: __('Always', 'we-icon-blocks'),
            value: 'always'
          }, {
            label: __('On hover', 'we-icon-blocks'),
            value: 'hover'
          }],
          onChange: value => setAttributes({
